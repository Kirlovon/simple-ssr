...
module.exports = class {
	...
	static puppeteerConfig(config, logs) {
		...
		if (logs && typeof config !== 'undefined' && typeof config !== 'object') {
	...
	static renderURL(url, logs) {
		...
		if (logs && typeof url === 'undefined') {
			warn(`The URL must be specified!`);
		}

		if (logs && typeof url !== 'undefined' && typeof url !== 'string') {
	...
	static renderConfig(config, logs) {
		...
		if (typeof config.domTarget === 'undefined' || typeof config.domTarget !== 'string') {
			config.domTarget = undefined;
		}

		if (typeof config.waitUntil === undefined || (typeof config.waitUntil !== 'string' && typeof config.waitUntil !== 'object')) {
			config.waitUntil = 'networkidle0';
		}

		if (typeof config.cache === 'undefined' || typeof config.cache !== 'boolean') {
			config.cache = false;
		}

		if (typeof config.cacheTime === 'undefined' || typeof config.cacheTime !== 'number') {
