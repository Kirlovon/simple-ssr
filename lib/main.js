...
class simpleSSR {
	...
	constructor() {
		...
		this.detailedErrors = false; // Enable of Disable errors from Puppeteer
	...
	async start(config) {
		...
		try {
			...
			if (this.browser != undefined) {
				throw new Error('Puppeteer has already been launched!');
			}
		...
		try {
			...
			try {
				...
				if (this.detailedErrors) {
					...
					throw new Error(`Failed to find "${config.domTarget}" in "${url}"!`);
				}
			}
			...
			try {
				...
				if (this.detailedErrors) {
					...
					throw new Error(`Error finishing rendering "${url}"!`);
				}
			}
