...
class simpleSSR {
	...
	async render(url, config) {
		...
		try {
			...
			if (typeof url === 'undefined' && this.safe) {
				throw new Error('URL for rendering is not specified!');
			}

			// If browser not started
			if (typeof this.browser === 'undefined' && this.safe) {
			...
			try {
				if (typeof config.domTarget !== 'undefined') {
	...
	async stop() {
		try {
			...
			if (typeof this.browser === 'undefined' && this.logs) {
